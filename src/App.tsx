...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Check each section to find the currently active one
      sections.forEach((sectionId, index) => {
        ...
        if (element) {
          ...
          // For the last section (contact), activate it when it's visible at all
          if (isLastSection && rect.top <= window.innerHeight * 0.5) {
            currentSection = sectionId;
          }
          // For other sections, use the standard logic
          else if (!isLastSection && rect.top <= 100 && rect.bottom >= 100) {
            currentSection = sectionId;
          }
        }
      });
      
      // If we're near the bottom of the page, always activate the last section
      const isNearBottom = window.innerHeight + window.scrollY >= document.body.offsetHeight - 100;
      if (isNearBottom) {
        currentSection = sections[sections.length - 1]; // 'contact'
      }
    ...
    <div className="App">
      ...
      <aside className={`social-sidebar ${showSocialSidebar ? 'social-sidebar--visible' : ''}`}>
        <div className="social-sidebar__container">
          {/* Temporarily disabled - no active social media accounts yet
          <a 
